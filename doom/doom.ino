...
void IRAM_ATTR TaskEngine(void * pvParameters) {
    ...
    for (;;) {
        ...
        if (hp <= 0) {
            ...
            tft.setCursor(80 - tft.textWidth("GAME OVER") / 2, 23);
            tft.print("GAME OVER");
            ...
            tft.setTextColor(COL_WHITE);
            {
                ...
                tft.setCursor(80 - tft.textWidth(gbuf) / 2, 42);
                tft.print(gbuf);
            }
            int16_t menuX = 80 - tft.textWidth("[A] Play Again") / 2;
            tft.setCursor(menuX, 57);
            tft.print("[A] Play Again");
            tft.setCursor(menuX, 70);
            tft.print("[B] Game Menu");
            tft.setCursor(menuX, 83);
            tft.print("[C] OS Menu");
            playSound(NOTE_E3, 150);

            // Olum anindaki A basili kalintisini yut: once birakilmasini bekle.
            vTaskDelay(pdMS_TO_TICKS(1500));
            while (!digitalRead(BTN_A) || !digitalRead(BTN_B) || !digitalRead(BTN_C)) { vTaskDelay(pdMS_TO_TICKS(50)); }
            ...
            while (1) {
                ...
                if (!digitalRead(BTN_B)) {          // [B] Game Menu
                    playSound(NOTE_G4, 50);
                    delay(200);
                    gameState = STATE_MENU;
                    menuDrawn = false;
                ...
                if (!digitalRead(BTN_C)) {          // [C] OS Menu → launcher'a don
                    playSound(NOTE_G4, 50);
                    delay(200);
                    returnToOS();
                }
        ...
        if (levelDone) {
            ...
            if (victory) {
                ...
                int16_t menuX = 80 - tft.textWidth("[A] Play Again") / 2;
                tft.setCursor(menuX, 74); tft.print("[A] Play Again");
                tft.setCursor(menuX, 86); tft.print("[B] Game Menu");
                tft.setCursor(menuX, 98); tft.print("[C] OS Menu");
            ...
            while (1) {
                ...
                if (!digitalRead(BTN_C)) {   // [C] OS Menu → launcher
                    returnToOS();
                }
                vTaskDelay(pdMS_TO_TICKS(100));
            }
